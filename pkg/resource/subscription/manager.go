...
// +kubebuilder:rbac:groups=sns.services.k8s.aws,resources=subscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sns.services.k8s.aws,resources=subscriptions/status,verbs=get;update;patch
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.PendingConfirmation == nil {
		return false, nil
	}
	pendingConfirmationCandidates := []string{"false"}
	if !ackutil.InStrings(*r.ko.Status.PendingConfirmation, pendingConfirmationCandidates) {
		return false, nil
	}

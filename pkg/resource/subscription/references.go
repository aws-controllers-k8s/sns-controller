...
func (rm *resourceManager) ResolveReferences(
	...
	if err == nil {
		err = resolveReferenceForTopicARN(ctx, apiReader, namespace, ko)
	}
...
// resource
func hasNonNilReferences(ko *svcapitypes.Subscription) bool {
	return false || (ko.Spec.TopicRef != nil)
}

// resolveReferenceForTopicARN reads the resource referenced
// from TopicRef field and sets the TopicARN
// from referenced resource
func resolveReferenceForTopicARN(
	ctx context.Context,
	apiReader client.Reader,
	namespace string,
	ko *svcapitypes.Subscription,
) error {
	if ko.Spec.TopicRef != nil && ko.Spec.TopicRef.From != nil {
		arr := ko.Spec.TopicRef.From
		if arr == nil || arr.Name == nil || *arr.Name == "" {
			return fmt.Errorf("provided resource reference is nil or empty: TopicRef")
		}
		obj := &svcapitypes.Topic{}
		if err := getReferencedResourceState_Topic(ctx, apiReader, obj, *arr.Name, namespace); err != nil {
			return err
		}
		ko.Spec.TopicARN = (*string)(obj.Status.ACKResourceMetadata.ARN)
	}
